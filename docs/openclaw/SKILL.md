...
Use AgentScope (`ascope`) whenever you need protected access to local macOS apps.

## Purpose

AgentScope is the approved path for OpenClaw to access sensitive local applications
...
- macOS automation permission is granted to AgentScope, not improvised per command
...
- Always use `ascope`; do not call AppleScript directly.
...
```bash
ascope notes read_note --agent openclaw --folder "Work" --note "Sprint Plan"
```
...
5. If the broker appears unavailable, check `ascope status` and `ascope doctor`.
...
- Do not claim Calendar or other apps are available unless `ascope app list` shows them
  or the user has explicitly configured them.
- Do not assume AgentScope labels are secrets; use only the label the user provisioned
  for OpenClaw.
